import PostStars from '@/components/shared/PostStars'
            ...
            <div className='flex-between w-full'>
              < Link to={`/profile/${post?.creator.$id}`} className='flex items-center gap-3'>
                <img src={ post?.creator?.imageUrl|| "assets/icons/profile-placeholder.svg"}
                  alt="creator" className='w-8 h-8 rounded-full lg:w-12 lg:h-12'/>
              ...
              <div className='flex-center'>
                <Link to= {`/update-post/${post.$id}`} className={`${user.id!==post?.creator.$id &&'hiden'}`}>
                  <img src='/assets/icons/edit.svg' width={24}height={24} alt='edit'/>
                </Link>

                <Button onClick={handleDeletePost} variant='ghost' 
                  className={`ghost_details-delete_btn $${user.id!==post?.creator.$id &&'hiden'}`}>
